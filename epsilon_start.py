...
# epsilon_final: 0.001
# epsilon_start: 0.250
param1 = {'epsilon_start': 0.750, 'epsilon_final': 0.001, 'epsilon_decay': 50000}
param2 = {'epsilon_start': 0.250, 'epsilon_final': 0.001, 'epsilon_decay': 50000}
param3 = {'epsilon_start': 0.005, 'epsilon_final': 0.001, 'epsilon_decay': 50000}
epsilon_params = [param1, param2, param3]
param1 = list()
param2 = list()
param3 = list()

for epsilon_param in epsilon_params :
    ...
    while True:
        ...
        epsilon = epsilon_by_frame(frame_idx, epsilon_param)
        ...
        if is_done:
            ...
            if len(all_rewards) > 10 and len(losses) > 10:
                ...
                if epsilon_params[0]['epsilon_start'] == epsilon_param['epsilon_start']:
                    param1.append(r100)
                elif epsilon_params[0]['epsilon_start'] == epsilon_param['epsilon_start']:
                    param2.append(r100)
                else:
                    param3.append(r100)
...
plt.plot(param1, label=f'Start = {epsilon_params[0]["epsilon_start"]}')
plt.plot(param2, label=f'Start = {epsilon_params[1]["epsilon_start"]}')
plt.plot(param3, label=f'Start = {epsilon_params[2]["epsilon_start"]}')
plt.title('Cartpole-V1 R100 against Episode Number for Epsilon Start')
plt.xlabel('Episode')
plt.ylabel('R100')
plt.legend()
plt.grid(True)
plt.savefig('results/cartpole_v1_epsilon_start.png')
plt.show()
