...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--network", default='dqn', help="DQN type - one of dqn, double-dqn and duelling-dqn")
        ...
        while True:
            ...
            if reward is not None:
                ...
                if best_mean_reward is None or best_mean_reward < mean_reward:
                    torch.save(net.state_dict(), f"{params['save_path']}/{args.env}-best-{learning_rate}.dat")
    ...
    plt.plot(single_r100, label=f'LR = {learning_rates[0]}')
    plt.plot(double_r100, label=f'LR = {learning_rates[1]}')
    plt.plot(duelling_r100, label=f'LR = {learning_rates[2]}')
    plt.title('Pong R100 against Frame for Varying Learning Rates')
    plt.xlabel('Frame')
    plt.ylabel('R100')
    plt.legend()
    plt.grid(True)
    plt.savefig('results/pong-lr-r100.png')